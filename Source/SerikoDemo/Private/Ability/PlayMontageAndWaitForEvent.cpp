...
#include "Ability/PlayMontageAndWaitForEvent.h"
...
URPGAbilitySystemComponent* UPlayMontageAndWaitForEvent::GetTargetASC()
{
...
{
	if (Ability && Ability->GetCurrentMontage())
	{
...
{
   UAbilitySystemGlobals::NonShipping_ApplyGlobalAbilityScaler_Duration(Rate);
...
void UPlayMontageAndWaitForEvent::Activate()
{
	...
	if (RPGAbilitySystemComponent)
	{
	  ...
	  if (AnimInstance != nullptr)
	  {
		  ...
		  if (RPGAbilitySystemComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfoRef(), MontageToPlay, Rate, StartSection) > 0.f)
		  {
