...
FRPGGameplayEffectContainerSpec URPGGameplayAbility::MakeEffectContainerSpecFromContainer(const FRPGameplayEffectContainer& Container, const FGameplayEventData& EventData, int32 OverrideGameplayLevel/*=-1*/)
{
    ...
    if (OwningASC)
    {
       // If we have a target type,run the targeting logic. This is optional, targets can be added later
        if (Container.TargetType.Get())
        {
           ...
           const FGameplayTagContainer& IgnoreTagContainer=Container.IgnoreTagContainer;
           AActor* AvatarActor=GetAvatarActorFromActorInfo();
           TargetTypeCDO->GetTargets(OwningCharacter,AvatarActor,EventData,IgnoreTagContainer,HitResults,TargetActors);
           ReturnSpec.AddTargets(HitResults,TargetActors);
        } 

        // If we dont's have an override level, use the default on the ability itself
...
{
   FRPGameplayEffectContainer* FoundContainer=EffectContainerMap.Find(ContainerTag);
